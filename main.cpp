...
#define IKATP 0 // ATP-sensitive potassium currentr
...
const int iso = 0;
FILE*t_,*v_,*nai_,*nass_,*ki_,*kss_,*cai_,*cass_,*cansr_,*cajsr_,*Jrel_,*CaMKt_,*Jup_,*Jtr_,*Jdiff_,*JdiffNa_,*JdiffK_,*Jleak_,*INa_,*Ito_,*ICaL_,*ICaNa_,*ICaK_,*IKr_, *IKur_, *IKs_,*IK1_,*INaCa_i_,*INaCa_ss_,*INaCa_,*INaK_,*IKb_,*INab_,*IpCa_,*ICab_,*Ist_,*dt_,*APD_, *ENa_, *EK_, *EKs_,*f_,*d_,*nca_,*fca_,*jca_,*fp_,*fcap_,*tff_,*tfs_,*tffp_,*dss_, *fss_,*Ical_V;
...
void tss_xura(REAL ee, double *txura, double *xurass)
{
     *xurass=1.0/(1.0+exp((-(ee + 6))/8.6));
     *txura=0.5+9.0/(1.0+exp((ee + 5.0)/12.0));
}

void tss_xuri(REAL ee, double *txuri, double *xuriss)
{
     *xuriss=1.0/(1.0+exp(((ee + 7.5))/10.0));
     *txuri=3050+590/(1.0+exp((ee + 60.0)/10.0));
}
...
void tss_d(REAL ee, double *td, double *dss){
    if (iso==1)
    {
        ...
        //*dss=1.0/(1.0+exp((-(ee+3.940+9.9*c_ICaL_dV_act))/4.230)); //or
        //*dss=1.0/(1.0+exp((-(ee+11.4+9.9*c_ICaL_dV_act))/7.4)); //dyn
    }
    else 
		//*dss=1.0/(1.0+exp((-(ee+3.940))/4.230)); //or
		//*dss=1.0/(1.0+exp((-(ee+11.4))/7.4)); //dyn
		*dss=1.0/(1.0+exp((-(ee+11.4))/4.230)); // only V0.5
		
		*td=0.6+1.0/(exp(-0.05*(ee+6.0))+exp(0.09*(ee +14.0)));//or
		//*td=0.6+1.0/(exp(-0.05*(ee+7.46+6.0))+exp(0.09*(ee +7.46+14.0))); //dyn
}
...
void tss_ff(REAL ee, double *tff, double *fss){
    if (iso==1)
    {
        ...
        //*fss=1.0/(1.0+exp((ee +19.58+8.0*c_ICaL_dV_inact)/3.696));//or
        //*fss=1.0/(1.0+exp((ee +27.4+8.0*c_ICaL_dV_inact)/7.1)); //dyn
    }
    else 
		*fss=1.0/(1.0+exp((ee + 19.58)/3.696));//or
		//*fss=1.0/(1.0+exp((ee +27.4)/7.1)); //dyn
		//*fss=1.0/(1.0+exp((ee +27.4)/3.696)); //dyn only V0.5
    
		*tff=7.0+1.0/(0.0045*exp(-(ee+ 20.0)/10.0)+0.0045*exp((ee+ 20.0)/10.0));//or
		//*tff=7.0+1.0/(0.0045*exp(-(ee+7.46+ 20.0)/10.0)+0.0045*exp((ee+7.46+ 20.0)/10.0)); // dyn
}

void tss_fs(REAL ee, double *tfs){
	*tfs=1000.0+1.0/(0.000035*exp(-(ee+5.0)/4.0)+0.000035*exp((ee + 5.0)/6.0));//or
    //*tfs=1000.0+1.0/(0.000035*exp(-(ee+7.46+5.0)/4.0)+0.000035*exp((ee +7.46+ 5.0)/6.0)); // dyn
}
...
clock_t Ina_start, Ina_stop, Ito_start, Ito_stop, Ical_start, Ical_stop, Ikr_start, Ikr_stop,Ikur_start, Ikur_stop, Iks_start, Iks_stop, Ik1_start, Ik1_stop, Inaca_start, Inaca_stop, Inak_stop, Inak_start, Inab_stop, Inab_start, ical_test_start, ical_test_stop, ina_t_start, ina_t_stop, ina_wt_start, ina_wt_stop;
...
void RGC(struct State *CurrentState,  struct State *NextState, int z,  int chain_length)
{
    ...
    for (z=0; z<chain_length; z++)
    {
        ...
        INa = c_m_gnal* GNa * (CurrentState[z].v-ENa) * CurrentState[z].m * CurrentState[z].m * CurrentState[z].m * ((1.0-fINap) * h * CurrentState[z].j + fINap * hp * CurrentState[z].jp);
        ...
        Ito = c_m_gto * Gto * (CurrentState[z].v - EK) * ((1.0 - fItop) * CurrentState[z].a * i + fItop * CurrentState[z].ap * ip);
        Ito_stop = clock();
        ...
        CurrentState[z].fss=1.0/(1.0+exp((CurrentState[z].v + 19.58)/3.696));
        ...
        ICaL=c_m_pca * (1.0-fICaLp) * PCa * PhiCaL * CurrentState[z].d * (f * (1.0 - CurrentState[z].nca) + CurrentState[z].jca * fca * CurrentState[z].nca) + fICaLp * PCap * PhiCaL * CurrentState[z].d * (fp * (1.0-CurrentState[z].nca) + CurrentState[z].jca * fcap * CurrentState[z].nca);
        ICaNa= c_m_pca * (1.0-fICaLp) * PCaNa * PhiCaNa * CurrentState[z].d * (f * (1.0 - CurrentState[z].nca) + CurrentState[z].jca * fca * CurrentState[z].nca)+fICaLp * PCaNap * PhiCaNa * CurrentState[z].d * (fp * (1.0 - CurrentState[z].nca) + CurrentState[z].jca * fcap * CurrentState[z].nca);
        ICaK=c_m_pca * (1.0-fICaLp) * PCaK * PhiCaK * CurrentState[z].d * (f * (1.0 - CurrentState[z].nca) + CurrentState[z].jca * fca * CurrentState[z].nca)+fICaLp * PCaKp * PhiCaK * CurrentState[z].d * (fp * (1.0 - CurrentState[z].nca) + CurrentState[z].jca * fcap * CurrentState[z].nca);
        ...
        Ikr_start = clock();
            ...
            GKr *= c_gkr;
                ...
                IKr= c_m_gkr * GKr*sqrt(ko/5.4)*CurrentState[z].xrs*rkr*(CurrentState[z].v - EK);
            #else
                ...
                IKr= c_m_gkr * GKr*sqrt(ko/5.4)*xr*rkr*(CurrentState[z].v - EK);
            #endif
        #else
            double GKr = 0.046;
            #if IKR_GRANDI
        ...
        IKs = c_m_gks * pdgn * GKs * KsCa * CurrentState[z].xs1 * CurrentState[z].xs2 * (CurrentState[z].v-EKs);
        ...
        Ikur_start = clock();
        #if TABLE
            static double txura_t[NTABLE], xurass_t[NTABLE], txuri_t[NTABLE], xuriss_t[NTABLE];
        
            iter = 0;
        
            static int first_ikur = 1;
            for ( ; first_ikur; first_ikur = 0){
                for (iter = -1; ++iter<NTABLE; )
                {
                    ee = ETMIN + (ETMAX - ETMIN)*iter/(NTABLE-1);
                    tss_xura(ee, txura_t+iter, xurass_t+iter);
                    tss_xuri(ee, txuri_t+iter, xuriss_t+iter);
                }
            }
        
            double txura = txura_t[iii]+T1.frac*(txura_t[iii+1] - txura_t[iii]);
            double xurass = xurass_t[iii]+T1.frac*(xurass_t[iii+1] - xurass_t[iii]);
            double txuri = txuri_t[iii]+T1.frac*(txuri_t[iii+1] - txuri_t[iii]);
            double xuriss = xuriss_t[iii]+T1.frac*(xuriss_t[iii+1] - xuriss_t[iii]);
        #else
			double xurass=1.0/(1.0+exp((-(CurrentState[z].v + 6))/8.6));
        ...
        */
        ...
        NextState[z].xuri = xuriss - (xuriss - CurrentState[z].xuri) * exp(-dt/txuri);
	    ...
	    INaCa= c_m_gncx* (INaCa_i+INaCa_ss);
	    ...
	    INaK = c_m_pnak * Pnak * (zna * JnakNa + zk * JnakK);
	    ...
	    double GpCa=0.0005;
	    GpCa*=c_gpca;
	    IpCa = c_m_gpca * GpCa * CurrentState[z].cai/(0.0005 + CurrentState[z].cai);
        ...
        /* 15. Calculate IKatp*/
            ...
            IKatp = c_m_gkatp * sigma * GKatp * prob * f_atp * (CurrentState[z].v - EK);
        #endif
	 ...
	 //   if ((t>(start+n*CL) && t<(start+duration+n*CL-dt)))
       ...
       /*     #if IKATP
         ...
         #if CHAIN //? chain 1 check circule...
            #if IKATP
                ...
                if (z==0) NextState[z].v = CurrentState[z].v-dt*(INa+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IKur+IK1+INaCa+INaK+INab+IKb+IpCa+ICab+Ist+IKatp+I_gap_junc);
                else NextState[z].v = CurrentState[z].v-dt*(INa+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IKur+IK1+INaCa+INaK+INab+IKb+IpCa+ICab+IKatp+I_gap_junc);
            #else
                if(z<(chain_length-1))	I_gap_junc += -g_gap_junc*(CurrentState[z+1].v-CurrentState[z].v);
                if(z>0)			I_gap_junc += -g_gap_junc*(-CurrentState[z].v + CurrentState[z-1].v);
                if (z==0) NextState[z].v = CurrentState[z].v-dt*(INa+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IKur+IK1+INaCa+INaK+INab+IKb+IpCa+ICab+Ist+I_gap_junc);
	    ...
	    double Jupp;
             if (iso==1)
	    {
		 Jupnp = c_m_jup * c_jup * 0.004375 * CurrentState[z].cai/(CurrentState[z].cai + 0.00092*0.54*c_SERCA);
	    }
            else   Jupnp = c_m_jup * c_jup* 0.004375 * CurrentState[z].cai/(CurrentState[z].cai + 0.00092);

	  
        if (iso==1)
	    {
		 Jupp = c_m_jup * c_jup * 2.75 * 0.004375 * CurrentState[z].cai/(CurrentState[z].cai + (0.00092 - 0.00017)*0.54*c_SERCA);
	    }
            else   Jupp = c_m_jup * c_jup *2.75 * 0.004375 * CurrentState[z].cai/(CurrentState[z].cai + 0.00092 - 0.00017);
	    if (celltype==1)
	    {
		Jupnp*=1.3;
		Jupp*=1.3;
	    ...
	    NextState[z].nai = CurrentState[z].nai +dt * (-(INa+3.0*INaCa_i+3.0*INaK+INab)*Acap/(F*vmyo)+JdiffNa*vss/vmyo);
	    NextState[z].nass = CurrentState[z].nass +dt * (-(ICaNa+3.0*INaCa_ss)*Acap/(F*vss)-JdiffNa);
	    
	    NextState[z].ki = CurrentState[z].ki+ dt * (-(Ito+IKr+IKs+IKur+IK1+IKb+Ist-2.0*INaK)*Acap/(F*vmyo)+JdiffK*vss/vmyo);
	 ...
	 //   {
		Bcai=1.0/(1.0+ c_m_CMDN * c_CMDN*cmdnmax*kmcmdn/pow(kmcmdn + CurrentState[z].cai,2.0)+trpnmax*kmtrpn/pow(kmtrpn + CurrentState[z].cai,2.0));
	  //  }
	  //  else
	  //  {
...
void open(){
    ...
    IKur_= fopen("tests_ctrl/IKur.txt","w");
...
double nai0,nass0,ki0,kss0,cai0,cass0,cansr0,cajsr0,m0,hf0,hs0,jO,hsp0,jp0,mL0,hL0,hLp0,a0,iF0,iS0,ap0,iFp0,iSp0,d0,ff0,fs0,f0,fcaf0,fcas0,jca0,nca0,ffp0,fcafp0,xrf0,xrs0,xs10,xs20,xura0,xuri0, xk10,Jrelnp0,Jrelp0,CaMKt0;

//int main(int argc, char *argv[]) //both for step and basis
int main() //     COMMENT both for step and basis
{  
...
//		}
    ...
    #if ADAPTIVE_STEP
         if (t>=t_output)
            {   t_output+=0.01;//0.5  change in case of rare step of current
...
// for ICaL   
                //fprintf(f_,"%e\t",CurrentState[z].f);
                fprintf(d_,"%e\t",CurrentState[z].d);//lala
                //printf(nca_,"%e\t",CurrentState[z].nca);
                //fprintf(fca_,"%e\t",fca);
                //fprintf(jca_,"%e\t",CurrentState[z].jca);
                //fprintf(fp_,"%e\t",CurrentState[z].fp);
                fprintf(f_,"%e\t",CurrentState[z].f);
                //fprintf(fcap_,"%e\t",fcap);
                //fprintf(tff_,"%e\t",CurrentState[z].tff); 
                //fprintf(tfs_,"%e\t",CurrentState[z].tfs); 
...
// for ICaL   
      ...
      //int tmp1 = 0;
        ...
        if ((t > (np-1)*CL)&&(tmp1 == 0)){
            ...
            FILE *fout = fopen( "state_2000_IKR_from_ORd.dat", "w" );
			printf("ya tut");
            fwrite(&NextState[0], sizeof(struct State), 1, fout);
